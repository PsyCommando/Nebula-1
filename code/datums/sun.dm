#define SOLAR_UPDATE_TIME (60 SECONDS) //duration between two updates of the whole sun/solars positions
...
/datum/sun/New()
	...
	if(prob(50))                       // same chance to rotate clockwise than counter-clockwise
		rate = -rate
	solar_next_update = REALTIMEOFDAY  // init the timer
...
/datum/sun/proc/calc_position()
	...
	if(REALTIMEOFDAY < solar_next_update) //if less than 60 game secondes have passed, do nothing
		return
	...
	events_repository.raise_event(/decl/observ/sun_position_changed, angle)
